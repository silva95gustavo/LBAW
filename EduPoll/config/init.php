<?php
 	  session_start();
	  setcookie(session_name(),session_id(),time() + (365 * 24 * 60 * 60), '/~up201303313/EduPoll');
	  error_reporting(E_ERROR | E_WARNING); // E_NOTICE by default
	   
	  $BASE_DIR = '/usr/users2/mieic2013/up201303313/public_html/EduPoll/';
	  $BASE_URL = '/~up201303313/EduPoll/';
	  $conn = new PDO('pgsql:host=dbm;dbname=lbaw1565', 'lbaw1565', 'BN80V7U5');
	  $conn->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);
	  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	  //$conn->exec('SET SCHEMA \'Gustavo\'');
	  $conn->exec('SET SCHEMA \'Andre\'');
